...
- **Etapa 5 — Importação CSV e geração em lote**: entregue numa segunda rodada. Admin sobe um CSV, vê prévia, confirma, e recebe um resumo (processadas/criadas/já existentes/para revisão/erros) linha a linha — reaproveita a mesma validação de domínio e detecção de duplicidade do cadastro manual único.
- **Etapa 6 — Associação automática de produtos**: quando a linha do CSV não traz `part_slug`, `modules/affiliate/matching.ts` tenta associar por EAN → MPN → marca+modelo → nome aproximado, cada um com uma confiança própria. Acima de 90% a oferta é criada direto; entre 50% e 90% vira uma `OfferMatchReview` pendente em `/admin/afiliados/revisao` (aprovar cria a oferta, rejeitar descarta); abaixo de 50% é erro (nenhum candidato plausível).

- **Etapa 8 — Relatórios administrativos**: `/admin/afiliados/relatorios` traz cliques dos últimos 14 dias (gráfico simples), por dispositivo, por origem (página de produto/montador/jogo/outro), ofertas mais clicadas, distribuição de status das ofertas, ofertas desatualizadas (sem checar há mais de 7 dias) e peças do catálogo sem nenhuma oferta ativa. Relatório de conversões importadas não entrou — não há nenhuma fonte de conversão real para importar ainda (depende de um provedor automático).
